...
export const EggheadLegacyUserRowSchema = z.object({
	id: z.string(),
	name: z.string().nullable(),
	role: z.string(),
	email: z.string(),
	...
	legacyContactId: z.string().nullable(),
	emailVerified: z.date().nullable(),
	image: z.string().nullable(),
	createdAt: z.date().nullable(),
})
export type EggheadLegacyUserRow = z.infer<typeof EggheadLegacyUserRowSchema>

const eggheadLegacyUserRowSelection = {
	id: users.id,
	name: users.name,
	role: users.role,
	email: users.email,
	fields: users.fields,
	legacyRailsUserId: users.legacyRailsUserId,
	legacyContactId: users.legacyContactId,
	emailVerified: users.emailVerified,
	image: users.image,
	createdAt: users.createdAt,
} as const

export async function findUserByLegacyRailsUserId(
	legacyRailsUserId: string,
): Promise<EggheadLegacyUserRow | null> {
	const user = await db
		.select(eggheadLegacyUserRowSelection)
		.from(users)
		.where(eq(users.legacyRailsUserId, legacyRailsUserId))
		.limit(1)
		.then((rows) => rows[0] ?? null)

	return EggheadLegacyUserRowSchema.nullable().parse(user)
}

export async function findUserByLegacyContactId(
	legacyContactId: string,
): Promise<EggheadLegacyUserRow | null> {
	const user = await db
		.select(eggheadLegacyUserRowSelection)
		.from(users)
		.where(eq(users.legacyContactId, legacyContactId))
		.limit(1)
		.then((rows) => rows[0] ?? null)
