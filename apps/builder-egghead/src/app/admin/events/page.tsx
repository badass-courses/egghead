...
export default async function AdminEventsPage() {
	...
	try {
		const [loadedEvents, loadedActiveEvents, pastEventIds] = await Promise.all([
			getAllEvents(),
			getActiveEvents(),
			getPastEventIds(),
		])
		...
		activeEvents = loadedActiveEvents ?? []
		pastEvents = allEvents.filter((event) => pastEventIds.includes(event.id))
	} catch (error) {
