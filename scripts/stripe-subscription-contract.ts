...
try {
  const checks = [
    ...
    check(
      "membership discovery supports offered recurring prices without a product interval",
      () => {
        ...
        assert.equal(isActiveMembershipProduct(membership), true);
        assert.equal(
          isActiveMembershipProduct({ ...membership, prices: [{ ...quarterlyPrice, status: 0 }] }),
          false,
        );
        assert.equal(
          isActiveMembershipProduct({
            ...membership,
            prices: [
              {
                ...quarterlyPrice,
                fields: { ...quarterlyPrice.fields, offer: { offered: false } },
              },
            ],
          }),
          false,
            ...
            prices: [
              {
                ...quarterlyPrice,
                fields: {
                  ...quarterlyPrice.fields,
                  stripe: { ...quarterlyPrice.fields.stripe, recurring: null },
                },
              },
            ],
          }),
